...
// ─── Types (prop-ready for future dynamic content) ──────────────────────────
...
// ─── Hardcoded content (will become props) ──────────────────────────────────
function useDefaultSections(onBrowseRewards: () => void): WingmanSection[] {
  return [
    {
      label: 'WATCHING YOUR 6',
      insights: [
        {
    ...
    {
      label: 'YOUR AMBITIONS',
      insights: [],
      emptyState: 'Tell me what you\'re working toward.',
    },
  ];
}

// ─── Highlight bold values in text ──────────────────────────────────────────
function renderTextWithBold(text: string, boldValues?: string[]) {
...
// ─── Component ──────────────────────────────────────────────────────────────
export function WingmanFAB({ sections: sectionsProp }: WingmanFABProps) {
  ...
  const sections = sectionsProp ?? useDefaultSections(handleBrowseRewards);
        ...
        >
          ...
          <div
            style={{
              ...
              padding: '14px 18px',
              borderBottom: '1px solid rgba(90,90,88,0.15)',
            }}
          >
            <span
              style={{
            ...
            {sections.map((section, si) => (
              <div key={si} style={{ marginBottom: si < sections.length - 1 ? '20px' : 0 }}>
                ...
                >
                  ...
                  <div
                    style={{
                      flex: 1,
                      height: '1px',
                      background: 'rgba(226,255,109,0.15)',
                    }}
                  />
                </div>
                ...
                {section.insights.map((insight, ii) => {
                  const isOpp = insight.variant === 'opportunity';
                  return (
                    <div
                      key={ii}
                      style={{
                    ...
                    >
                      ...
                      >
                        {renderTextWithBold(insight.text, insight.boldValues)}
                      </p>
                      <span
                        style={{
                          fontFamily: "'DM Mono', monospace",
                ...
                {section.link && (
                  <button
                    ...
                    style={{
                      ...
                      background: 'none',
                      border: 'none',
                      borderBottom: '1px solid rgba(226,255,109,0.25)',
                      padding: '2px 0',
                      cursor: 'pointer',
                      marginTop: '4px',
                    }}
                ...
                {/* Empty state */}
                {section.emptyState && section.insights.length === 0 && (
                  <p
                    style={{
                      fontFamily: "'DM Sans', sans-serif",
                      fontSize: '12px',
