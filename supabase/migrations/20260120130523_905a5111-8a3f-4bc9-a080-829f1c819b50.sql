...
CREATE TABLE IF NOT EXISTS unified_profiles (
  ...
  garden_data jsonb DEFAULT '{}'::jsonb,  -- Garden-specific: referrals, earnings, etc.
  crescendo_data jsonb DEFAULT '{}'::jsonb, -- Crescendo-specific: claims_balance, role, etc.
...
CREATE TABLE IF NOT EXISTS wallet_portfolio (
  ...
  -- Lock details
  locks jsonb DEFAULT '[]'::jsonb, -- Array of {amount, lock_type, lock_date, unlock_date}
  
  -- Sync tracking
  last_synced_at timestamptz DEFAULT now(),
  sync_source text, -- 'manual', 'api', 'onchain'
...
CREATE TABLE IF NOT EXISTS cross_platform_activity_log (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid REFERENCES unified_profiles(id),
  platform text NOT NULL CHECK (platform IN ('garden', 'crescendo', 'admin')),
...
CREATE OR REPLACE FUNCTION get_user_with_tier(p_user_id uuid)
RETURNS jsonb AS $$
DECLARE
  v_result jsonb;
BEGIN
...
RETURNS TRIGGER AS $$
BEGIN
  PERFORM calculate_user_tier(NEW.user_id);
  RETURN NEW;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER SET search_path = public;

DROP TRIGGER IF EXISTS portfolio_tier_update ON wallet_portfolio;
CREATE TRIGGER portfolio_tier_update
...
CREATE INDEX IF NOT EXISTS idx_cross_platform_activity_user ON cross_platform_activity_log(user_id, created_at DESC);
...
ALTER TABLE cross_platform_activity_log ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "Anyone can read status tiers" ON status_tiers
  FOR SELECT USING (true);

-- Admins can manage status tiers
CREATE POLICY "Admins manage status tiers" ON status_tiers
  FOR ALL USING (has_role(auth.uid(), 'admin'::app_role));

-- Users can read their own portfolio
CREATE POLICY "Users read own portfolio" ON wallet_portfolio
  FOR SELECT USING (
    user_id IN (SELECT id FROM unified_profiles WHERE auth_user_id = auth.uid())
  );

-- Users can insert their own portfolio
CREATE POLICY "Users insert own portfolio" ON wallet_portfolio
  FOR INSERT WITH CHECK (
    user_id IN (SELECT id FROM unified_profiles WHERE auth_user_id = auth.uid())
  );

-- Users can update their own portfolio
CREATE POLICY "Users update own portfolio" ON wallet_portfolio
  FOR UPDATE USING (
    user_id IN (SELECT id FROM unified_profiles WHERE auth_user_id = auth.uid())
  );

-- Users can read their own activity
CREATE POLICY "Users read own cross platform activity" ON cross_platform_activity_log
  FOR SELECT USING (
    user_id IN (SELECT id FROM unified_profiles WHERE auth_user_id = auth.uid())
  );

-- System can insert activity logs
CREATE POLICY "System insert cross platform activity" ON cross_platform_activity_log
  FOR INSERT WITH CHECK (true);

-- Admins can read all activity
CREATE POLICY "Admins read all cross platform activity" ON cross_platform_activity_log
  FOR SELECT USING (has_role(auth.uid(), 'admin'::app_role));

-- Admins can read all settings
CREATE POLICY "Admins read admin settings" ON admin_settings
  FOR SELECT USING (has_role(auth.uid(), 'admin'::app_role));

-- Admins can manage settings
CREATE POLICY "Admins manage admin settings" ON admin_settings
  FOR ALL USING (has_role(auth.uid(), 'admin'::app_role));